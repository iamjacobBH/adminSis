...
then
    ...
    if [ $? -eq 0 ]; 
    then
        #mostramos los comandos ejecutados
        string="$(cat /home/$logname/.bash_history)"
    	dialog --title "Comandos ejecutados por el usuario $logname" --msgbox "$string" 22 70
    else
        dialog --title "Comandos ejecutados por el usuario $logname" --msgbox "\nEl usuario $logname no existe" 22 70
    fi
fi
